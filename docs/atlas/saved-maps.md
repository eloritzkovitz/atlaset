...
You can create a blank new map by clicking `Create new map` in the `Saved Maps` panel.

### 2. Saving a shared map

You can also save shared maps by clicking the `Save map` in the `Saved Maps` panel, while browsing a shared map.
