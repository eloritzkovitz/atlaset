...
You can open the `My Maps` panel from the `map toolbar` or by pressing the <kbd>B</kbd> key.

## Creating new maps

There are two methods of creating new maps:

### 1. Creating a new map

You can create a blank new map by clicking `Create new map` in the `My Maps` panel.

### 2. Saving a shared map

You can also save shared maps by clicking the `Save map` in the `My Maps` panel, while browsing a shared map.
