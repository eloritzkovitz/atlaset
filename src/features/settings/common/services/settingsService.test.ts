...
describe("settingsService", () => {
  beforeEach(() => {
    if (!appDb.settings) {
      throw new Error(
        "appDb.settings is undefined. The mock was not set up correctly.",
      );
    }
    Object.values(appDb.settings).forEach((fn) =>
  ...
  it("loads settings from db if present (guest)", async () => {
    ...
    (
      appDb.settings.get as unknown as ReturnType<typeof vi.fn>
